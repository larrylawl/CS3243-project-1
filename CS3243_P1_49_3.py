...
class Puzzle(object):
    ...
    def explore_next_states(self, node, queue, puzzle):
        ...
        for action in puzzle.valid_actions(node):
            ...
            if state_to_string(child_node["state"]) not in puzzle.past_states:
                heuristic = puzzle.get_heuristic_Euclidean(child_node["state"])
    ...
    # Manhattan heuristic
    def get_heuristic_Euclidean(self, state):
        no_of_rows = self.k
        index = 0
        euclidean_sum = 0
        ...
        for val in state:
            if (val != 0):
                curr_row, curr_col = puzzle.convert_val_to_coord(index)                    
                goal_row, goal_col = puzzle.convert_val_to_coord(val - 1)
