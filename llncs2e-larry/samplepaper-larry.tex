...
\textbf{Rules}~\cite{aditya_goel_how_nodate}. Let \( M \) denote a \( k \times k \) matrix, \( m_{i, j} \) denote a blank tile in \( M \), and \( n_i \) denote the number of inversions in the intial state \( M_{initial} \). Puzzle is solvable if (1) \( k \) is odd and \( n_i \) is even or (2) \( k \) is even, the sum of \(n_i \) and \( i \) is odd.
...
\begin{enumerate}
    ...
    \]
\end{enumerate}
\textbf{Prove for Consistency.} Refer to Appendix \ref{appendix:linear_conflict}.
...
\textbf{Time complexity} is measured by the number of nodes generated during the search (ie number of the explored states) \cite[p80]{rosalind_euclidean_nodate}. \\
...
\begin{center}
    \includegraphics[width=\textwidth]{fig1_update.png}
    % \captionof{Performance of the three different heuristics under varying levels of k-puzzle difficulty.} \label{fig1}
\end{center}
...
For step values above 15, we observe that M and L outperforms E in terms of time and space complexity. This is in line with how E will always be dominated by M and L, explaining the difference performance in time and space complexity.\\ \\
In addition, for step values higher than 22, we observe that L outperforms M in terms of both time and space complexity. This is also in line with how L dominates M which translates directly into time and space efficiency (p104).\\ \\
\begin{center}
    \includegraphics[width=\textwidth]{actual_time.jpg}
    % \caption{Performance of the three different heuristics under varying levels of k-puzzle difficulty.} \label{fig2}
\end{center}
In terms of actual running time (in seconds), it is interesting to note that while both M and L outperforms E, 
L actually takes longer time to solve the puzzle as compared to when M is used. 
This is because of the extra time required to calculate L for each node before it is put in the frontier. \\ \\
In conclusion, our experiment has shown that more dominant heuristics prove to be more efficient 
in terms of time and space complexity. However, in terms of actual time taken, a better heuristic 
is not guaranteed to be faster as it is ultimately dependent on the amount of actual time needed to compute the heuristic itself.
...
\begin{proof} Proof by Construction. \\
    To prove consistency, we must prove that for all \( s \) and \( s' \), \( f(s') \geq f(s) \), where \( s' \) is the successor of \( s \).
\[
    f(s) = g(s) + h(s)
\]
