...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Let \( lc(s, r_i) \) denote the number of tiles that must be removed from row \( r_i \) in order to resolve the linear conflicts in \( r_i \).
    \end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Condition 2:} \( r_j \) is the goal row of \( t_j \).
        ...
        \( MD(s') = MD(s) - 1 \). Since \( r_i \) is not the goal row of \( t_j \), \( lc(s', r_i) = lc(s, r_i) \). 
