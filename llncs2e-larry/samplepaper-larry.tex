...
%
\institute{INSERT DALIS MATRIC NUMBER \and
...
% TODO Add in proves for heuristic
...
\textbf{Prove for Consistency.} Euclidian distance is a form general triangle inequality, given that the euclidian distance from start state \( S \) to end state \( G \) (1 side of the triangle) cannot be longer than the sum of the 2 sides ( the actual distance from S to middle state N and the euclidian distance from N to G) as the euclidian distance from \( S \) to \( G \) is already the shortest path. 
...
\textbf{Prove for Consistency.} Prove for consistency can be found in appendix \ref{appendix:manhat_cons}.
...
\appendix
\section{Prove for Manhattan Distance Consistency}
